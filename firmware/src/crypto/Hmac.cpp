...
void block(Sha256Ctx &c, const uint8_t *p) {
    uint32_t w[64];
    for (int i = 0; i < 16; ++i)
...
void update(Sha256Ctx &c, const uint8_t *data, size_t len) {
    c.len += len;
    while (len > 0) {
        const size_t take = (64 - c.n < len) ? (64 - c.n) : len;
...
void sha256(const uint8_t *data, size_t len, uint8_t out[32]) {
    Sha256Ctx c; init(c); update(c, data, len); final(c, out);
}
